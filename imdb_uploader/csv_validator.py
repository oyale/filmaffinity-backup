...
COLUMN_VARIANTS = {
    ...
    'original_title': ['original title', 'original_title', 'originaltitle', 'titulo original', 'título original'],
}
...
def _find_column(columns: list[str], canonical_name: str) -> Optional[str]:
    ...
    try:
        with open(path, newline='', encoding='utf-8') as fh:
            ...
            if empty_title_count > 0:
                result.add_warning(
                    f"{empty_title_count} row(s) have empty title (will be skipped)"
                )
        ...
        with open(path, newline='', encoding='utf-8') as fh:
            ...
            for req_col in LETTERBOXD_REQUIRED_COLUMNS:
                ...
                if not any(col in variants for col in normalized_columns):
                    result.add_error(
                        f"Missing required column '{display_name}'"
                    )
            ...
            for rec_col in LETTERBOXD_RECOMMENDED_COLUMNS:
                ...
                if not any(col in variants for col in normalized_columns):
                    result.add_warning(
                        f"Missing recommended column '{display_name}'"
                    )
            ...
            for row in reader:
                ...
                if rows_checked <= max_preview_rows:
                    ...
                    rating_value = (
                        normalized_row.get('rating10') or
                        normalized_row.get('rating') or ''
                    ).strip()
                    if rating_value:
                        try:
            ...
            if empty_title_count > 0:
                result.add_warning(
                    f"{empty_title_count} row(s) have empty title"
                )

            if invalid_rating_count > 0:
                result.add_warning(
                    f"{invalid_rating_count} row(s) have invalid rating "
                    "(should be 0.5-10)"
                )
